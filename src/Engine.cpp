...
std::vector<Vec2D> getFigursToMove(const Field &field, bool wikingsToMove) {
    ...
    for (int x = 0; x < FIELD_SIZE; x++) {
        for (int y = 0; y < FIELD_SIZE; y++) {
            if ((wikingsToMove && field[x][y] == Figur::Wiking) ||
                (!wikingsToMove && (field[x][y] == Figur::Guard || field[x][y] == Figur::King))) {
...
void insertAvailableMovesFigurInDirection(std::vector<Move> &availableMoves, const Field &field,
    ...
    while (0 <= position.x && position.x < FIELD_SIZE && 0 <= position.y &&
           position.y < FIELD_SIZE) {
...
EvaluatedMovePath Engine::minimax(Game game, Move move, unsigned int depth, int alpha, int beta) {
    if (move.from.x != -1) {
        ...
        if (winner == Figur::Wiking) {
            ...
            std::cout << "wikingsCount: " << game.getWikingCount() << ",
            guardsCount: " << game.getGuardCount() << std::endl;
            game.printField();
            */
...
Engine::Engine(Game &game, unsigned int maxDepth) : game(game), maxDepth(maxDepth) {
...
Move Engine::getMove() {
    ...
    for (unsigned int depth = 1; depth < maxDepth + 1; depth++) {
        evaluatedMovePath =
            minimax(game, move, depth, alpha_beta_starting_value * -1, alpha_beta_starting_value);
...
{ game.move(move); game.updateField(move.to); Figur winner = game.whoWon(); if
(winner == Figur::Wiking) { game.printField(); return 1000; } else if (winner ==
Figur::King) { game.printField(); return -1000;
    }
    ...
    if (game.areWikingsToMove()) {
        int max = -1000;
        std::vector<Vec2D> figuresToMove = getFigursToMove(field,
game.areWikingsToMove()); std::vector<Move> availableMoves =
getAvailableMoves(field, figuresToMove); for (Move move : availableMoves) { int
evaluation = minimaxHelper(game, move, depth - 1, alpha, beta); max =
std::max(max, evaluation); alpha = std::max(alpha, evaluation); if (beta <=
alpha) { break;
            }
        }
        return max;
...
game.areWikingsToMove()); std::vector<Move> availableMoves =
getAvailableMoves(field, figuresToMove); for (Move move : availableMoves) { int
evaluation = minimaxHelper(game, move, depth - 1, alpha, beta); min =
std::min(min, evaluation); beta = std::min(beta, evaluation); if (beta <= alpha)
{ break;
            }
        }
        return min;
