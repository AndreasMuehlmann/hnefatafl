...
NegamaxNoAlphaBeta::NegamaxNoAlphaBeta(unsigned int thinkingTimeMs)
    : m_thinkingTimeMs(thinkingTimeMs), m_maxDepth(std::numeric_limits<unsigned int>::max()) {}

NegamaxNoAlphaBeta::NegamaxNoAlphaBeta(unsigned int thinkingTimeMs, unsigned int maxDepth)
    : m_thinkingTimeMs(thinkingTimeMs), m_maxDepth(maxDepth) {}

auto NegamaxNoAlphaBeta::getMove(const Game &game) -> Move {
...
auto NegamaxNoAlphaBeta::negamax(Game &game, Move move, unsigned int depth) -> EvaluatedMove {
    if (move.from != FIELDS) {
